...
export default {
  ...
  methods: {
    setLocal() {
      if (!!this.content) {
        localStorage.setItem("enContent", this.content);
      } else {
        localStorage.setItem("enContent", "");
      }
    },
    onEditorBlur() {
      // 失去焦点事件
    ...
    onEditorReady(dom) {
      // 异步原因所以要再请求一次渲染
      this.content = mockData.encontent;
      // this.$axios.get("/static/contentData.json").then(res => {
      //   this.content = res.data.encontent;
      // });
    },
    // 富文本图片上传前
    beforeUpload() {
    ...
    uploadSuccess(res, file) {
      ...
      // res为图片服务器返回的数据
      // 获取富文本组件实例
      let quill = this.$refs.enquill.quill;
      // 如果上传成功
      // if (res.code === 200) {
      if (res.result) {
        // 获取光标所在位置
